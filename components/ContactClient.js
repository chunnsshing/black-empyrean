...
function Toast({ msg, show, error }) {
  return (
    <div className={'toast' + (show ? ' show' : '') + (error ? ' toast-error' : '')}>
      <span className="check">{error ? '✕' : '✓'}</span> {msg}
    </div>
  );
}
...
function ContactForm({ onSuccess, onError }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      onSuccess("Message sent — we'll be in touch within 24 business hours");
  ...
  return (
    <form onSubmit={handleSubmit} noValidate>
      ...
      <button type="submit" className="btn btn-primary" style={{ marginTop: 8, opacity: loading ? 0.7 : 1 }} disabled={loading}>
        {loading ? 'Sending…' : <><span>Send Message</span> <span className="btn-arrow">→</span></>}
      </button>
    </form>
  );
}
...
export default function ContactClient() {
  ...
  return (
    <>
      ...
      <Toast msg={toast.msg} show={toast.show} error={toast.error} />
    </>
  );
}
